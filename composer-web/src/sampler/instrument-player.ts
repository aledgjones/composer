...
export class InstrumentPlayer {
   ...
   */
  private expressions: Map<number, ExpressionPlayer> = new Map();
  ...
  public async load(expression: number, url: string, progress: Progress) {
    ...
    this.expressions.set(expression, player);
  }
  ...
  public play(
    ...
    if (this.expressions.has(expression)) {
      this.expressions.get(expression)?.play(pitch, when, duration);
    } else {
      // play using standard Expression.Natural
      this.expressions.get(0)?.play(pitch, when, duration);
    }
  }
   ...
   */
  public stopAll() {
    this.expressions.forEach((player) => {
      player.stopAll();
    });
  }
